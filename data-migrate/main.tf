...
resource "aws_instance" "data_migrate_server" {
  ...
  vpc_security_group_ids = [var.data_migration_server_security_group_id]
  iam_instance_profile   = var.ec2_instance_instance_profile_role_name
  ...
  user_data_base64 = base64encode(templatefile("${path.module}/db-migrate-script.sh.tpl", {
    ...
    RDS_DB_NAME       = var.rds_db_secret_name
